...
help:
	...
	@echo "  pkg-devstack               build the channel-messaging image from the latest configuration and code"
...
test: clean
	coverage run ./manage.py test channel-messaging --settings=channel-messaging.settings.test
	coverage report

quality:
	pep8 --config=.pep8 channel-messaging *.py
	pylint --rcfile=pylintrc channel-messaging *.py
...
dummy_translations:
	cd channel-messaging && i18n_tool dummy
...
open-devstack:
	docker exec -it channel-messaging /edx/app/channel-messaging/devstack.sh open

pkg-devstack:
	docker build -t channel-messaging:latest -f docker/build/channel-messaging/Dockerfile git://github.com/edx/configuration
